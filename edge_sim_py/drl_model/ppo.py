...
class PPO:
    ...
    def update(self, transition_dict):
        ...
        #     next_v = torch.tensor(transition_dict['next_v']).view(-1, 1).to(self.device)
        #
        #     dones = torch.tensor(transition_dict['dones'],
        #                          dtype=torch.float).view(-1, 1).to(self.device)
        #
        #     #TODO：TD无法也应当从样本中获取其中self.critic(next_states)即为V(St+1)
        ...
        #     actor_loss = -torch.mean(torch.min(surr1, surr2))  # PPO损失函数,梯度上升
        #
