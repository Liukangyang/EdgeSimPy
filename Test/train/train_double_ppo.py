...
from edge_sim_py.drl_model import DoublePPO
import numpy as np
import random
import torch
import os
...
def Stop_func() -> bool:
    return all(user.task_count > int(config['double-ppo']['MAX_STEPS_PER_EPISODE']) for user in MyUser.all())
...
agent_params=config['double-ppo']
#构建智能体
    ...
    for i in range(50):
        with tqdm(total=int(num_episodes / 50), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes/50)):
                ...
                #在线策略
                transition_dict = {'states': [], 'action1': [],'action2':[],'next_states': [], 'rewards': [], 'dones': []}
                ...
                while not done:
                    ...
                    transition_dict['action1'].append(combined_action[0])
                ...
                episode_losses_custom.append(avg_actor_loss)
                pbar.update(1)
                # 每 10 次迭代打印一次进度
